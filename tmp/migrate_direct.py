...
3. Bypasses Django's serialization/deserialization to avoid the print statement issues

Usage:
    python tmp/migrate_direct.py
"""
import os
import sys
import sqlite3
...
def verify_environment():
    ...
    if not env_link.is_symlink():
        print("ERROR: .env is not a symlink. Run ./tmp/switch_env.py prod first.")
        return False

    # Check if linked to prod.mac.env
    if not os.readlink(env_link).endswith("prod.mac.env"):
        print("ERROR: Not using production environment. Run ./tmp/switch_env.py prod first.")
        return False
